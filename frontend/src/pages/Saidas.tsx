...
import type { Localizacao, Movimentacao, Produto, ResumoSaldoOut } from "../types";
...
export function Saidas() {
  ...
  const [saldoSelecionado, setSaldoSelecionado] = useState<ResumoSaldoOut | null>(null);
  ...
  useEffect(() => {
    ...
    produtosApi
      .consultarSaldo(Number(produtoId))
      .then(setSaldoSelecionado)
      .catch(() => setSaldoSelecionado(null));
  }, [produtoId]);
  ...
  function validate(): boolean {
    ...
    else if (saldoSelecionado && qtd > saldoSelecionado.quantidade_total) {
      next.quantidade = `Quantidade insuficiente. Disponível: ${saldoSelecionado.quantidade_total} unidade(s).`;
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await movimentacoesApi.registrarSaida({
      ...
      showToast(`Saída de ${quantidade} unidade(s) de "${produto?.nome}" registrada com sucesso!`, "success");
      setQuantidade("");
      setMotivo("");
      setErrors({});
      await loadData();

      const novoSaldo = await produtosApi.consultarSaldo(Number(produtoId));
      setSaldoSelecionado(novoSaldo);
      if (novoSaldo.abaixo_minimo) {
        setTimeout(() => {
          showToast(
            `Atenção: estoque de "${produto?.nome}" abaixo do mínimo! (${novoSaldo.quantidade_total} de ${novoSaldo.estoque_minimo} mín.)`,
            "warning"
          );
        }, 400);
      }
      ...
      if (err instanceof ApiError) {
        if (err.status === 400) {
          setErrors({ quantidade: err.message });
        } else {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Registrar Saída</h2>
        <p className="text-gray-500 text-sm mt-1">Registre a saída de produtos do estoque</p>
      </div>

      <div className="bg-white rounded-xl shadow p-6 mb-8">
        <h3 className="text-gray-700 font-semibold mb-5">Nova Saída</h3>
        <form onSubmit={handleSubmit} noValidate>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              {saldoSelecionado && (
                <p className={`text-xs mt-1 ${saldoSelecionado.quantidade_total === 0 ? "text-red-500 font-medium" : "text-gray-500"}`}>
                  Disponível em estoque: {saldoSelecionado.quantidade_total} unidade(s)
                  {saldoSelecionado.quantidade_total === 0 && " — sem saldo para saída"}
                </p>
              )}
            </div>
            <div>
      ...
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-gray-700 font-semibold">Histórico de Saídas</h3>
                ...
                historico.map((h, i) => (
                  <tr key={h.id} className={(i % 2 === 0 ? "bg-white" : "bg-gray-50") + " hover:bg-blue-50 transition-colors"}>
                    ...
                    <td className="py-3 px-2 text-center">
                      <span className="bg-red-100 text-red-700 text-xs font-bold px-2.5 py-0.5 rounded-full">-{h.quantidade}</span>
                    </td>
